...
# ---------------------------------------------------------------------------
# Chromium / CEF backend for wxWebView
# ---------------------------------------------------------------------------
# The upstream webview_chromium/CMakeLists.txt tries to download a CEF binary
# distribution or use a local one via add_subdirectory(). In vcpkg we already
# have a pre-built CEF overlay port, so we replace that file with a minimal
# version that uses find_package(CEF CONFIG) instead.
if("webview-chromium" IN_LIST FEATURES)
    file(WRITE "${SOURCE_PATH}/build/cmake/lib/webview_chromium/CMakeLists.txt"
[=[
#############################################################################
# webview_chromium/CMakeLists.txt  (vcpkg overlay – uses pre-built CEF)
#############################################################################

# Locate the CEF package installed by the vcpkg overlay port.
find_package(CEF CONFIG REQUIRED)

# wxWidgets source files use:  #include "include/cef_client.h"
# CEF::wrapper exposes <prefix>/include as its INTERFACE_INCLUDE_DIRECTORIES,
# but wxWidgets needs the *parent* of that directory on the search path so
# that "include/cef_client.h" resolves correctly.
get_target_property(_cef_include_dirs CEF::wrapper INTERFACE_INCLUDE_DIRECTORIES)
list(GET _cef_include_dirs 0 _cef_inc)
get_filename_component(_cef_prefix "${_cef_inc}" DIRECTORY)

set(CEF_ROOT "${_cef_prefix}" CACHE PATH "CEF root directory (vcpkg)" FORCE)
wx_lib_include_directories(wxwebview PRIVATE "${_cef_prefix}")

# Link the webview library against the complete CEF package.
wx_lib_link_libraries(wxwebview PUBLIC CEF::CEF)
]=])
endif()
...
vcpkg_cmake_configure(
    ...
    MAYBE_UNUSED_VARIABLES
        CMAKE_DISABLE_FIND_PACKAGE_GSPOLL
